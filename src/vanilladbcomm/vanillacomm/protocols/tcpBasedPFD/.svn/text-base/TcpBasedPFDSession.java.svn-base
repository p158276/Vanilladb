...
public class TcpBasedPFDSession extends Session {
    ...
    private void notifyCrash(TcpUndeliveredEvent event) {
        if (started) {
            SampleProcess p = processes.getProcess((SocketAddress) event
                    .getFailedAddress());
            if (p.isCorrect()) {
                p.setCorrect(false);

                try {
                    Crash crash = new Crash(channel, Direction.UP, this,
                            p.getProcessNumber());
                    crash.go();
                } catch (AppiaEventException e) {
                    e.printStackTrace();
                ...
                try {
                    Crash crash = new Crash(channel, Direction.DOWN, this,
                            p.getProcessNumber());
                    crash.go();
                } catch (AppiaEventException e) {
                    e.printStackTrace();
