 ...
 */
package vanillacomm.protocols.allAckURB;
...
import vanillacomm.protocols.utils.MessageEntry;
import vanillacomm.protocols.utils.MessageID;
...
public class AllAckURBSession extends Session {
    private static int DELIVERED_SHRINK_SIZE = 100;
    ...
    private boolean canDeliver(MessageEntry entry) {
        ...
        for (int i = 0; i < procSize; i++)
            if (processes.getProcess(i).isCorrect() && (!entry.acks[i]))
                return false;
        return ((old_delivered[entry.messageID.process] < entry.messageID.seqNumber) && (!delivered.contains(entry.messageID)) && received
    ...
    private void handleProcessInitEvent(ProcessInitEvent event) {
        ...
        old_delivered = new int[processes.getSize()];
    ...
    private void shrinkDelivered(MessageID mid){
        int pid = mid.process;
        int sn = mid.seqNumber;
        int old_sn = old_delivered[pid];
