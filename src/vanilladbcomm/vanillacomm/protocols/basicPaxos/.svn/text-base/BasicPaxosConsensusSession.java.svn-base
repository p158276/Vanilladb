...
public class BasicPaxosConsensusSession extends Session {
    ...
    private void handlePaxosPropose(PaxosPropose pp){
        ...
        // bebBroadcast
        try {
            Read ev = new Read(pp.getChannel(),
                    Direction.DOWN, this);
    ...
    private void handleRead(Read read){
        ...
        if(rts >= ts || wts >= ts){
            ...
            // pp2p
            try {
                Nack ev = new Nack(read.getChannel(),
                        Direction.DOWN, this);
                ev.source = correct.getSelfProcess();
                ev.dest = read.source;
            ...
            // pp2p
            try {
                ReadAck ev = new ReadAck(read.getChannel(),
                        Direction.DOWN, this);
    ...
    private void handleNack(Nack nack){
        try {
            PaxosReturn ev = new PaxosReturn(nack.getChannel(),
                    Direction.UP, this);
    ...
    private void handleWrite(Write write){
        ...
        if(rts >= ts || wts >= ts){
            ...
            // pp2p
            try {
                Nack ev = new Nack(write.getChannel(),
                        Direction.DOWN, this);
                ev.source = correct.getSelfProcess();
            ...
            // pp2p
            try {
                WriteAck ev = new WriteAck(write.getChannel(),
                        Direction.DOWN, this);
    ...
    private void writeDecide(Channel channel){
        if(wAcks > correct.getSize()/2){
            ...
            try {
                PaxosReturn ev = new PaxosReturn(channel,
                        Direction.UP, this);
